...
type result = Winner of player | Remi
...
let other_player player: player = match player with
| X -> O
| O -> X
...
let place_token player grid (row_i, col_i) : state =
  ...
  | None (* grid is full *) -> 
      ...
      GameOver {result = Remi; final_grid = updated_grid}
  | Some player ->
...
let show_player player = if player = X then "X" else "O"
